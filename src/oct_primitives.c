...
static oct_Bool createPrimitiveType(oct_Context* ctx, oct_Type* t, oct_Uword size, oct_Uword alignment) {
	t->variant = OCT_TYPE_STRUCT;
	t->structType.size = size;
	t->structType.alignment = alignment;
